    ...
    B::Error: std::fmt::Display,
{
    ...
    {
        let bundle_root_buf: std::path::PathBuf = state.runner_host.bundle_root().to_path_buf();
        let session_store =
        ...
        let resp =
            crate::oauth_callback::handle_oauth_callback(ctx, provider_id, query_string).await;
        return Ok(resp);
    }
