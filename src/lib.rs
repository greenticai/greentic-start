...
fn run_start(mut request: StartRequest) -> anyhow::Result<()> {
    // `--store-root` boots from an operator-provided env home; a `--bundle` /
    // `--config` names the legacy single-bundle path. They select mutually
    // exclusive boot sources, so reject the combination up front — before the
    // operator logger or any config resolution runs — rather than silently
    // preferring one.
    ...
    // `--store-root` points at an operator-provided env home that already
    // exists (staged by greentic-deployer); we serve it in place and must not
    // synthesize a `local` env under the default `~/.greentic` root.
    if request.store_root.is_none() {
    ...
    if bundle_less {
        let env_id = resolve_env(request.env.as_deref());
        // `--store-root` overrides the default `~/.greentic` root so an operator
        // can serve an env home staged at an arbitrary path; without it we fall
        // back to the home-rooted default store.
