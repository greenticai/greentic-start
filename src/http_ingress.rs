...
async fn handle_request_inner(
    ...
    if domain == Domain::Events && !result.events.is_empty() {
        // Run event routing on a dedicated thread to avoid tokio nested-runtime
        // panics from greentic_runner_desktop::run_pack_with_options which calls
        // block_on internally.
        ...
        std::thread::spawn(move || {
                ...
                crate::event_router::route_events_to_default_flow(&bundle, &ctx, &events)
            {
                crate::operator_log::warn(
                    module_path!(),
                    format!("event routing failed: {err:#}"),
                );
            }
        });
    }
